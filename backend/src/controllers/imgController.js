...
class imgController {
  ...
  static async uploadProductImage(req, res) {
    try {
      ...
      // Subimos el buffer a Cloudinary en la carpeta product_images
      const result = await new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          { folder: "product_images" }, // carpeta específica para productos
          (error, result) => {
            if (error) reject(error);
            else resolve(result);
          }
      ...
      // Devolvemos la URL pública de la imagen del producto
      res.status(200).json({ 
        status: "Success",
